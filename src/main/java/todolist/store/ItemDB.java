...
public class ItemDB {
    ...
    public static List<Item> getItemList() {
        return ModelDB.tx(session -> session.createQuery("FROM todolist.model.Item").list());
    }

    public static List<Item> getActualItemList() {
        return ModelDB.tx(session -> session.createQuery("FROM todolist.model.Item WHERE done = null").list());
    }
}
