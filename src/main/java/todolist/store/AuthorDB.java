...
public class AuthorDB {
    ...
    public static Author get(String email) {
        return ModelDB.tx(session -> {
            Query<Author> q = session.createQuery("FROM todolist.model.Author WHERE email = ?1");
    ...
    public static List<Author> getAll() {
        return ModelDB.tx(session -> session.createQuery("FROM todolist.model.Author").list());
    }
}
